...
- Đã thêm seed idempotent `bun run seed:test --confirm-test-data` cho 240 tân cử nhân thử nghiệm, 4 hội trường, 6 phiên, 6 ảnh có sẵn và 5 account đủ vai trò `MN`/`CK`/`MC`/`US`/`NO`; mật khẩu test được sinh trong `.env.development.local` đã Git-ignore; đã chạy thành công trên PostgreSQL Docker local (160 đã check-in, 80 chưa check-in) và xác minh login đủ 5 account.
