...
export default function NotificationDisplayPage() {
    ...
    const [completedNotifications, setCompletedNotifications] = useState<any[]>([]); // Các thông báo đã phát
    const [error, setError] = useState<string | null>(null);
    const [currentTime, setCurrentTime] = useState(new Date());
    const [maxVisibleNotifications, setMaxVisibleNotifications] = useState(5); // Số thông báo tối đa hiển thị
    const [lastCompletedNotification, setLastCompletedNotification] = useState<any>(null); // Thông báo cuối cùng đã phát

    // Error boundary effect
    useEffect(() => {
        const handleError = (event: ErrorEvent) => {
            console.error('Global error caught:', event.error);
            setError(`Lỗi không xác định: ${event.error?.message || 'Unknown error'}`);
        };

        const handleUnhandledRejection = (event: PromiseRejectionEvent) => {
            console.error('Unhandled promise rejection:', event.reason);
            setError(`Lỗi promise: ${event.reason?.message || 'Unknown promise error'}`);
        };
    ...
    let connection, isConnected, connectionState, isFullscreen, toggleFullscreen;
    try {
    ...
    // TTS hook with error handling
    let speak: any, stop: any;
    try {
        const ttsHook = useElevenLabsTTS((reason) => {
        ...
        setError('TTS Hook initialization failed');
    ...
    useEffect(() => {
        const calculateMaxNotifications = () => {
            ...
            const notificationHeight = 140; // Tăng chiều cao ước tính để an toàn hơn
            const headerHeight = 80; // Tăng chiều cao header để bao gồm padding và margin
            const bottomPadding = 40; // Padding dưới cùng
            ...
            // Giới hạn từ 2-6 thông báo để đảm bảo không tràn
            const finalCount = Math.max(2, Math.min(maxNotifications, 6));
            console.log(`[Screen] Screen height: ${screenHeight}px, Available: ${availableHeight}px, Max notifications: ${finalCount}`);
        ...
        return () => window.removeEventListener('resize', calculateMaxNotifications);
    }, []);
    ...
    useEffect(() => {
        const checkOverflow = () => {
            ...
            if (container && completedNotifications.length > 0) {
                ...
                console.log(`[Overflow Check] Container height: ${containerHeight}px, Scroll height: ${containerScrollHeight}px`);

                if (containerScrollHeight > containerHeight + 10) { // +10px tolerance
                    console.warn(`[Overflow] Container overflow detected. Reducing notifications from ${maxVisibleNotifications} to ${maxVisibleNotifications - 1}`);
                    // Giảm số lượng thông báo hiển thị nếu bị tràn
                    setMaxVisibleNotifications(prev => Math.max(2, prev - 1));
                }
            }
        };

        // Kiểm tra sau khi render và animation hoàn thành
        ...
        if (connection) {
            connection.on('ReceiveTTSBroadcast', (data: any) => {
                ...
                const notification = {
                    ...
                    repeatCount: data.repeatCount || 1, // Số lần lặp lại
                    currentRepeat: 0, // Số lần đã phát
                    id: `${data.notificationId || Date.now()}-${Math.random()}`,
                };

                // Nếu có thông báo hiện tại đang hiển thị, đẩy nó vào hàng đợi
                if (lastCompletedNotification) {
                    ...
                    setQueue(prev => {
                        ...
                        console.log(`[Queue] Added current + new notification to queue. New queue length: ${sorted.length}`);
                        return sorted;
                    });
                    setLastCompletedNotification(null); // Clear thông báo hiện tại
                } else {
                    // Thêm vào queue và sắp xếp theo priority
                    setQueue(prev => {
                        const updated = [...prev, notification];
                        const sorted = sortNotifications(updated);
                        console.log(`[Queue] Added notification to queue. New queue length: ${sorted.length}`);
                        return sorted;
                    });
                }
            });
    ...
    const readNotificationWithRepeat = useCallback(async (notification: any, currentRepeat: number) => {
        console.log(`Reading notification: ${notification.title}, lần ${currentRepeat + 1}/${notification.repeatCount}`);

        // Xóa khỏi queue khi phát lần đầu (card trong hàng đợi biến mất)
        if (currentRepeat === 0) {
            setQueue(prev => {
                const filtered = prev.filter(item => item.id !== notification.id);
                console.log(`[Queue] Removed notification from queue. Remaining queue length: ${filtered.length}`);
                return filtered;
            });
        }

        setIsReading(true);
        setNotifications([notification]); // Hiển thị notification đang phát
        setCurrentIndex(0);
        ...
        try {
            ...
            // Kiểm tra xem đã phát đủ chưa
            if (nextRepeat >= notification.repeatCount) {
                // Đã phát xong, clear notifications và thêm vào danh sách đã phát
                setIsReading(false);
                setNotifications([]); // Clear notifications để có thể xử lý thông báo tiếp theo

                // Lưu thông báo cuối cùng đã phát
                setLastCompletedNotification({ ...notification, completedAt: new Date().toISOString() });

                // Thêm vào danh sách các thông báo đã phát
                setCompletedNotifications(prev => {
                    const updated = [...prev, { ...notification, completedAt: new Date().toISOString() }];
                    // Giới hạn theo kích thước màn hình, giữ lại thông báo mới nhất
                    return updated.length > maxVisibleNotifications ? updated.slice(-maxVisibleNotifications) : updated;
                });
            } else {
                // Chưa đủ, phát lại
                setIsReading(false);
    ...
    const processNextInQueue = useCallback(async () => {
        if (queue.length > 0 && !isReading) {
            console.log(`[Queue] Processing next notification. Queue length: ${queue.length}, isReading: ${isReading}`);
            const nextNotification = queue[0];
            await readNotificationWithRepeat(nextNotification, 0);
        } else {
            console.log(`[Queue] Cannot process. Queue length: ${queue.length}, isReading: ${isReading}`);
        }
    }, [queue, isReading, readNotificationWithRepeat]);

    // Auto process queue when new item added or when previous finished
    useEffect(() => {
        console.log(`[Queue Effect 1] Queue: ${queue.length}, isReading: ${isReading}, notifications: ${notifications.length}`);
        if (queue.length > 0 && !isReading && notifications.length === 0) {
            console.log('[Queue Effect 1] Triggering processNextInQueue');
            // Xử lý ngay lập tức khi có thông báo mới
            processNextInQueue();
        }
    ...
    useEffect(() => {
        console.log(`[Queue Effect 2] Queue: ${queue.length}, isReading: ${isReading}, notifications: ${notifications.length}`);
        if (queue.length > 0 && !isReading && notifications.length === 0) {
    ...
    const getCardShadow = (priority: number) => {
        switch (priority) {
            case 1: // CAO - Shadow đỏ
                return 'shadow-2xl shadow-red-500/50';
            case 2: // TRUNG BÌNH - Shadow vàng
                return 'shadow-2xl shadow-amber-500/50';
            case 3: // THẤP - Shadow xanh lá
    ...
    const getBorderStyle = (priority: number) => {
        switch (priority) {
            case 1: // CAO - Border đỏ
                return 'border-red-500';
            case 2: // TRUNG BÌNH - Border vàng
                return 'border-amber-500';
            case 3: // THẤP - Border xanh lá
        ...
        return (
            <div className="bg-gradient-to-br from-slate-950 via-slate-900 to-black h-full w-full flex items-center justify-center min-h-screen">
                <div className="text-center text-white p-8">
                    <div className="mb-6">
                        <Icon icon="heroicons-outline:exclamation-triangle" className="w-16 h-16 mx-auto text-red-400 mb-4" />
                    </div>
    ...
    return (
        <div className={`bg-gradient-to-br from-slate-950 via-slate-900 to-black h-full w-full relative overflow-hidden min-h-screen`}>
            {/* Background */}
            ...
            {isFullscreen && (
                <div className="fixed top-3 left-3 sm:top-8 sm:left-8 z-50">
                    ...
                    >
                        <Icon icon="heroicons-outline:clock" className="w-8 h-8 sm:w-5 sm:h-5" />
                    ...
                    <div className="col-span-1 lg:col-span-4 flex flex-col justify-center items-center w-full relative pb-32">
                        <div className="w-full max-w-4xl px-2 sm:px-0 flex-1 flex items-center justify-center">
                            ...
                            <AnimatePresence mode="wait" initial={false}>
                                {currentNotification && (
                                    ...
                                    >
                                        <Card className={cn(
                                            ...
                                            currentNotification ? getCardBackground(currentNotification.priority) : "bg-white",
                                            currentNotification ? getBorderStyle(currentNotification.priority) : "border-gray-300"
                                        )}>
                                            {/* Animated header bar */}
                                            <div className={cn(
                                                "absolute top-0 left-0 w-full h-1 transition-all duration-1000",
                                                currentNotification?.priority === 1 && "bg-gradient-to-r from-red-400 via-red-500 to-red-600 animate-pulse",
                                                currentNotification?.priority === 2 && "bg-gradient-to-r from-amber-400 via-orange-500 to-amber-600 animate-pulse",
                                                currentNotification?.priority === 3 && "bg-gradient-to-r from-green-400 via-emerald-500 to-green-600 animate-pulse"
                                            )}></div>

                                            {/* Glow effect */}
                                            <div className={cn(
                                                "absolute inset-0 rounded-3xl opacity-20 blur-3xl transition-all duration-1000",
                                                currentNotification?.priority === 1 && "bg-gradient-to-r from-red-400 to-red-600",
                                                currentNotification?.priority === 2 && "bg-gradient-to-r from-amber-400 to-orange-500",
                                                currentNotification?.priority === 3 && "bg-gradient-to-r from-green-400 to-emerald-500"
                                            )}></div>
                                            ...
                                            >
                                                ...
                                                <motion.div
                                                    ...
                                                    className={cn(
                                                        ...
                                                        currentNotification?.priority === 1 && "border-red-500/40",
                                                        currentNotification?.priority === 2 && "border-amber-500/40",
                                                        currentNotification?.priority === 3 && "border-emerald-500/40",
                                                        !currentNotification && "border-gray-200/40"
                                                    )}
                                                >
                                                    {/* Left: THÔNG BÁO PHIÊN TRAO, Scope & Hall */}
                                                    <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 sm:gap-3 w-full sm:w-auto">
                                                        {/* THÔNG BÁO PHIÊN TRAO Badge */}
                                                        <motion.div
                                                            animate={{
                                                                scale: [1, 1.05, 1],
                                                            }}
                                                        ...
                                                        >
                                                            ...
                                                            ></motion.div>
                                                            <span className="font-bold text-xs sm:text-sm tracking-wider text-gray-800">THÔNG BÁO</span>
                                                        </motion.div>

                                                        {/* Scope Badge - Enhanced Gradient */}
                                                        ...
                                                        {currentNotification?.hallName && (
                                                            <div className="px-3 py-1.5 sm:px-5 sm:py-2.5 bg-gradient-to-r from-slate-600 via-slate-700 to-slate-800 text-white rounded-full font-bold text-xs sm:text-sm tracking-wide hover:from-slate-500 hover:via-slate-600 hover:to-slate-700 shadow-lg shadow-slate-500/40 transition-all duration-300 backdrop-blur-sm flex items-center gap-1.5 sm:gap-2">
                                                                <Icon icon="heroicons-outline:building-office" className="w-3 h-3 sm:w-4 sm:h-4" />
                                                                {currentNotification.hallName}
                                                            </div>
                                                        )}
                                                    </div>
                                                    ...
                                                    <div className="flex items-center gap-2 sm:gap-3">
                                                        {/* Repeat Counter - Enhanced Gradient */}
                                                        ...
                                                        <div className="space-y-4 sm:space-y-6">
                                                            ...
                                                            >
                                                                <h2 className="text-2xl sm:text-4xl md:text-5xl lg:text-6xl font-black leading-tight text-gray-900 mb-3 sm:mb-4 tracking-tight">
                                                                    {currentNotification.title || currentNotification.content}
                                                                </h2>
                                                            ...
                                                            {/* White Content Box like in image */}
                                                            <motion.div
                                ...
                                {/* Display thông báo cuối cùng khi hàng đợi hết */}
                                {!currentNotification && queue.length === 0 && !isReading && lastCompletedNotification && (
                                    <motion.div
                                        key={`last-${lastCompletedNotification?.notificationId || Date.now()}`}
                                        initial={{
                                            opacity: 0,
                                            scale: 0.7,
                                            y: 80,
                                        ...
                                        animate={{
                                            opacity: 1, // Hiển thị sáng như bình thường
                                            scale: 1,
                                            y: 0,
                                            rotateX: 0,
                                            rotateY: 0
                                    ...
                                    >
                                        <Card className={cn(
                                            "relative overflow-hidden rounded-3xl border-2 transition-all duration-1000 ease-out backdrop-blur-sm",
                                            lastCompletedNotification ? getCardBackground(lastCompletedNotification.priority) : "bg-white",
                                            lastCompletedNotification ? getBorderStyle(lastCompletedNotification.priority) : "border-gray-300"
                                        )}>
                                            {/* Animated header bar */}
                                            <div className={cn(
                                                "absolute top-0 left-0 w-full h-1 transition-all duration-1000",
                                                lastCompletedNotification?.priority === 1 && "bg-gradient-to-r from-red-400 via-red-500 to-red-600 animate-pulse",
                                                lastCompletedNotification?.priority === 2 && "bg-gradient-to-r from-amber-400 via-orange-500 to-amber-600 animate-pulse",
                                                lastCompletedNotification?.priority === 3 && "bg-gradient-to-r from-green-400 via-emerald-500 to-green-600 animate-pulse"
                                            )}></div>

                                            {/* Glow effect */}
                                            <div className={cn(
                                                "absolute inset-0 rounded-3xl opacity-20 blur-3xl transition-all duration-1000",
                                                lastCompletedNotification?.priority === 1 && "bg-gradient-to-r from-red-400 to-red-600",
                                                lastCompletedNotification?.priority === 2 && "bg-gradient-to-r from-amber-400 to-orange-500",
                                                lastCompletedNotification?.priority === 3 && "bg-gradient-to-r from-green-400 to-emerald-500"
                                            )}></div>
                                            ...
                                            >
                                                ...
                                                <motion.div
                                                    ...
                                                    className={cn(
                                                        "flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 p-4 sm:p-6 bg-white/80 backdrop-blur-sm border-b-2 transition-all duration-300",
                                                        lastCompletedNotification?.priority === 1 && "border-red-500/40",
                                                        lastCompletedNotification?.priority === 2 && "border-amber-500/40",
                                                        lastCompletedNotification?.priority === 3 && "border-emerald-500/40",
                                                ...
                                                <CardContent className="p-4 sm:p-8 md:p-12">
                                                    {lastCompletedNotification && (
                                                        <div className="space-y-4 sm:space-y-6">
                                                            ...
                                                            >
                                                                <h2 className="text-2xl sm:text-4xl md:text-5xl lg:text-6xl font-black leading-tight text-gray-900 mb-3 sm:mb-4 tracking-tight">
                                                                    {lastCompletedNotification.title || lastCompletedNotification.content}
                                                                </h2>
                                                            ...
                                                            >
                                                                <div className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm text-gray-500">
                                                                    <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-blue-500"></div>
                                                                    {new Date(lastCompletedNotification.createdAt).toLocaleString('vi-VN')}
                                                                </div>
                                                            </motion.div>
                                                        </div>
                                                    )}
                                ...
                                {/* Display khi không có notification và không có trong queue */}
                                {!currentNotification && queue.length === 0 && !isReading && !lastCompletedNotification && (
                                    <motion.div
                                        initial={{ opacity: 0 }}
                                        animate={{ opacity: 1 }}
                                            ...
                                            <CardContent className="p-6 sm:p-8 md:p-12 text-center">
                                                <div className="mb-3 sm:mb-4 md:mb-6 flex justify-center">
                                                    ...
                                                    >
                                                        ...
                                                        />
                                                        <Icon icon="heroicons-outline:bell-slash" className="h-8 w-8 sm:h-12 sm:w-12 md:h-16 md:w-16 text-slate-300 relative z-10" />
                                                    </motion.div>
                                                </div>
                                                    ...
                                                    {isConnected ? (
                                                        <div className="flex items-center gap-1.5 sm:gap-2 px-3 py-1.5 sm:px-4 sm:py-2 md:px-6 md:py-3 bg-green-500/20 text-green-300 border border-green-500/40 shadow-lg shadow-green-500/20 backdrop-blur-sm rounded-full">
                                                            <div className="relative">
                                                                ...
                                                                <Icon icon="heroicons-outline:signal" className="h-3.5 w-3.5 sm:h-4 sm:w-4 md:h-5 md:w-5 relative" />
                                                            </div>
                                                            <span className="font-medium text-xs sm:text-sm">Đã kết nối</span>
                                                        </div>
                                                    ) : (
                                                        <div className="flex items-center gap-1.5 sm:gap-2 px-3 py-1.5 sm:px-4 sm:py-2 md:px-6 md:py-3 bg-blue-500/20 text-blue-300 border border-blue-500/40 shadow-lg shadow-blue-500/20 backdrop-blur-sm rounded-full">
                                                            <div className="relative">
                                                                <Icon icon="heroicons-outline:arrow-path" className="h-3.5 w-3.5 sm:h-4 sm:w-4 md:h-5 md:w-5 relative animate-spin" />
                                                            </div>
                                                            <span className="font-medium text-xs sm:text-sm">Đang kết nối...</span>
                                                        </div>
                                                    )}
                                                </div>
                                            </CardContent>
                        ...
                        {/* Hall Info Component - Bottom Left - Horizontal Layout - Only show when there are halls */}
                                            ...
                                            {halls.map((hall) => {
                                                return (
                                                    <motion.div
                                                        ...
                                                        className="bg-gradient-to-br from-slate-800/80 to-slate-900/90 backdrop-blur-sm rounded-xl p-4 border border-slate-600/40 min-w-[220px] flex-shrink-0 shadow-xl hover:shadow-2xl transition-all duration-300"
                                                    >
                                                        {/* Header với tên hall */}
                                                        ...
                                                        <div className="text-center space-y-3">
                                                            ...
                                                            <div className="inline-flex items-center justify-center">
                                                                <div className="bg-gradient-to-r from-blue-500 to-cyan-500 text-white px-4 py-2 rounded-full shadow-lg">
                                                                    <span className="text-sm font-bold">PHIÊN SỐ {hall.sessionNumber}</span>
                                                                </div>
                                                            </div>
                    ...
                    <div className="col-span-1 lg:col-span-1 hidden lg:flex flex-col">
                        <div className="bg-gradient-to-br from-slate-800/50 via-slate-800/50 to-slate-900/50 backdrop-blur-md rounded-2xl sm:rounded-3xl border border-slate-600/40 p-3 sm:p-4 h-full overflow-hidden flex flex-col">
                            ...
                            <div className="flex items-center justify-between gap-2 mb-3 pb-3 border-b border-slate-500/40">
                                <h3 className="text-sm font-bold text-slate-200">Thông Báo Đã Phát</h3>
                            ...
                            <div className="flex-1 overflow-hidden pr-2 notification-container" style={{ direction: 'rtl' }}>
                                <div className="space-y-3" style={{ direction: 'ltr' }}>
                                    {completedNotifications.length > 0 ? (
                                        <AnimatePresence mode="popLayout" initial={false}>
                                            {completedNotifications.slice().reverse().slice(0, maxVisibleNotifications).map((item, index) => {
                                                const isOverflow = completedNotifications.length > maxVisibleNotifications && index === maxVisibleNotifications - 1;
                                                return (
                                                    <motion.div
                                                        key={item.id}
                                                        ...
                                                        className={cn(
                                                            ...
                                                            item.priority === 1 && "bg-red-50/10 border-red-500/30 hover:bg-red-50/20 hover:border-red-500/50",
                                                            item.priority === 2 && "bg-yellow-50/10 border-amber-500/30 hover:bg-yellow-50/20 hover:border-amber-500/50",
                                                            item.priority === 3 && "bg-green-50/10 border-emerald-500/30 hover:bg-green-50/20 hover:border-emerald-500/50"
                                                        )}
                                                    >
                                                        <div className="flex items-center justify-between mb-2">
                                                            <Badge className={cn(
                                                                "text-xs px-3 py-0.5 rounded-full",
                                                                item.priority === 1 && "bg-red-500/20 text-red-300",
                                                                item.priority === 2 && "bg-amber-500/20 text-amber-300",
                                                                item.priority === 3 && "bg-emerald-500/20 text-emerald-300"
                                                            )}>
                                                            ...
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                <Icon icon="heroicons-outline:check-circle" className="w-3 h-3 text-green-400" />
                                                            </div>
                                                        </div>
                                            ...
                                            {/* Hiển thị indicator khi có thông báo bị ẩn */}
                                            {completedNotifications.length > maxVisibleNotifications && (
                                                <motion.div
                                                ...
                                                >
                                                    <div className="text-xs text-slate-400 bg-slate-700/50 rounded-lg px-3 py-2">
                                                        +{completedNotifications.length - maxVisibleNotifications} thông báo khác
                                                    </div>
                                                </motion.div>
                                            )}
                                        </AnimatePresence>
                                        ...
                                        <div className="text-center py-4 text-slate-400">
                                            <Icon icon="heroicons-outline:clock" className="w-8 h-8 mx-auto mb-2 opacity-50" />
