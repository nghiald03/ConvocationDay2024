...
export default function Page() {
  ...
  const {
    ...
    queryKey: ['images'],
    queryFn: async () => {
      const { data } = await uploader.get('/media', {
        params: { _: Date.now() },
      });
      return data.sort(
  ...
  const uploadMutation = useMutation({
    mutationFn: async (files: FileList) => {
      ...
      await uploader.post('/media/images/bulk', fd, {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => uploader.delete(`/media/${id}`),
    onSuccess: async () => {
  ...
  const bulkDeleteMutation = useMutation({
    mutationFn: async (ids: string[]) =>
      uploader.delete('/media', { data: { ids } }),
