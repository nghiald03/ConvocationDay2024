...
export default function NotificationDisplayLayout({
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            const token = session ? 'cookie-session' : '';
            if (token) {
                setSessionReady(true);

                // Parse role from token
                try {
                    const payload: Record<string, any> = { role: session?.role };
    ...
    const shouldInit = hubUrl && sessionReady && hubUrl.trim() !== '';
