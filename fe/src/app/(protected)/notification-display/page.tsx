...
const QUEUE_KEY = ['tts-queue'] as const;
const COMPLETED_KEY = ['tts-completed'] as const;
...
export default function NotificationDisplayPage() {
  ...
  const userRole = useSession()?.role ?? 'Unknown';
  ...
  const { data: halls = [], isLoading: hallLoading } = useQuery({
  ...
  const { speak: xiSpeak, stop: xiStop } = useElevenLabsTTS((reason) => {
  ...
  const readNotificationWithRepeat = useCallback(
    ...
    [speak, maxVisibleNotifications, queryClient]
  );
