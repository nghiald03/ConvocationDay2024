...
const QUERY_KEY = ['notify-messages'] as const;
...
export default function NotifyMockPage() {
  ...
  const items: NotifyMessage[] = useMemo(() => data?.data?.data ?? [], [data]);
  ...
  }, [consoleOnly, enabled, repeatCount, xiSpeak]);
  ...
  const createNotificationMutation = useMutation({
    mutationFn: async (params: {
      ...
      return createNotification(params.request);
    },
    onSuccess: async (response, { shouldSpeak, rawInputForSpeak }) => {
      toast.success(response.message || 'Tạo thông báo thành công!');

      if (shouldSpeak) {
        const temp: NotifyMessage = {
          id: response.notificationId ?? `POST-${Date.now()}`,
      ...
      <Card className='animate-fade-up'>
        <CardContent className='p-3'>
          ...
          <div className='mt-2 text-xs space-y-1'>
            ...
            <div>Phiên đăng nhập: cookie bảo mật</div>
          </div>
        </CardContent>
      </Card>
