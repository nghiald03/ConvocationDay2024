...
function SeatCell({
  ...
  const base =
    "relative flex items-center justify-center rounded-md text-[10px] sm:text-xs border select-none p-1.5 text-center transition-all duration-200";
  const tone =
    variant === "student"
      ? "bg-rose-50/80 border-rose-200/60 text-rose-700 hover:bg-rose-100/90 hover:border-rose-300"
      : "bg-sky-50/80 border-sky-200/60 text-sky-700 hover:bg-sky-100/90 hover:border-sky-300";

  const ring =
    occupant && !blockedOverlay ? "ring-1 ring-inset ring-white/50" : "";
  const interactive =
  ...
  const statusDot = occupant && !showDetails && !blockedOverlay && (
    <span
      className={`absolute right-1 top-1 h-2 w-2 rounded-full border border-white ${
        occupant.checkIn ? "bg-emerald-500" : "bg-rose-500"
      }`}
    />
  );

  const statusIcon = occupant && showDetails && !blockedOverlay && (
    <span
  ...
  const content = (
    ...
    >
      ...
      {showDetails ? (
        <div
          className={[
            "leading-tight font-medium flex items-center justify-center gap-1 sm:gap-2 text-center",
        ...
        >
          ...
          {occupant && !blockedOverlay ? (
            <span className="text-[10px]">
              ...
              <span className="opacity-70 shrink-0 text-[9px]">
        ...
        <div className="flex flex-col items-center gap-0.5">
          ...
          {occupant && !blockedOverlay && isHovered && (
            <div className="text-[8px] opacity-75 font-medium truncate max-w-full">
      ...
      {blockedOverlay && (
        <>
          <div className="pointer-events-none absolute inset-0 rounded-md bg-slate-200/40" />
          <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
            <span className="text-[14px] sm:text-base font-bold text-slate-600/80 tracking-wider">
              X
            </span>
          </div>
        </>
...
function BlockRow({
  ...
  const base =
    "relative flex items-center justify-center rounded-md text-[10px] sm:text-xs border select-none p-1.5 text-center";
  const color =
    tone === "student"
      ? "bg-rose-100/60 border-rose-300/60 text-rose-800"
      : "bg-sky-100/60 border-sky-300/60 text-sky-800";
    ...
    >
      {Array.from({ length: cols }).map((_, i) => {
        ...
        return (
          <div
            key={`ptop-x-${i}`}
            className="relative flex items-center justify-center rounded-md border p-1.5 text-center cursor-not-allowed bg-sky-100/60 border-sky-300/60 text-sky-800"
            style={xStyle}
            title="Khu vực không ngồi"
          >
            <span className="font-semibold">X</span>
...
const toneClass: Record<Tone, string> = {
  amber:
    "bg-slate-100/95 border-slate-300 text-slate-900 shadow-sm backdrop-blur-sm",
  sky: "bg-slate-100/95 border-slate-300 text-slate-900 shadow-sm backdrop-blur-sm",
  rose: "bg-slate-100/95 border-slate-300 text-slate-900 shadow-sm backdrop-blur-sm",
  slate:
    "bg-slate-100/95 border-slate-300 text-slate-900 shadow-sm backdrop-blur-sm",
};
...
function Pill({
  ...
  return (
    <div
      className={[
        "flex items-center justify-center gap-2 px-3 py-1.5 rounded-md border",
        toneClass[tone],
...
export default function SeatMapPage() {
  ...
  const {
    ...
    queryKey: ["seat-map-2", hall, session],
    queryFn: () =>
      getBachelors({
        pageIndex: 1,
        pageSize: 1000,
      ...
      {enabled && (
        <div className="flex gap-3 flex-wrap">
          <Badge
            color="default"
            rounded="lg"
            className="bg-rose-50 text-rose-700 border-rose-200 px-4 py-2"
          >
          ...
          <Badge
            color="default"
            rounded="lg"
            className="bg-sky-50 text-sky-700 border-sky-200 px-4 py-2"
          >
        ...
        <CardContent className="p-6">
          {!enabled ? (
            ...
            <div
              ...
              className={
                isFullscreen
                  ? "w-screen h-screen overflow-hidden bg-slate-50"
            ...
            >
              ...
              {isFullscreen ? (
                <div className="w-screen h-screen">
                  {/* Header */}
                  <div
                    className="flex items-center justify-center px-[5vw] bg-slate-50"
                    style={{ height: "10vh", width: "100vw" }}
                  >
                    <Badge
                      color="default"
                      rounded="lg"
                      className="bg-primary text-primary-foreground px-6 py-2 text-base font-bold cursor-pointer hover:bg-primary/90 transition-colors"
                      onClick={exitFull}
                    >
                  ...
                  <div>
                    ...
                    <div className="flex items-center justify-center mb-4 mt-2">
                      <Badge
                        color="default"
                        rounded="lg"
                        className="bg-primary text-primary-foreground px-4 py-2 font-bold"
                      >
                        SƠ ĐỒ HỘI TRƯỜNG {hallSymbol}
                      </Badge>
                      <Button
                    ...
                    >
                      ...
                      {/* LỐI GIỮA */}
                      <div
                        className="relative rounded-lg border border-dashed border-muted"
                        style={{
                          background:
