...
import { getBachelorsByHallSession } from '@/features/bachelor/api/get-bachelors-by-hall-session';
...
import UncheckedInTable from '@/features/statistics/ui/unchecked-in-table';
...
export default function HallOverviewPage() {
  const { data: halls = [], isLoading, isError } = useQuery({
    ...activeHallSummaryQueryOptions,
    refetchInterval: 5000,
    refetchIntervalInBackground: true,
  });

  const totalHalls = new Set(halls.map((hall) => hall.hallId)).size;
  const totalSessions = halls.length;
  const overview = useMemo(() => {
    const grouped = new Map<
      number,
      {
        ...
        sessions: ActiveHallSummary[];
      }
    >();

    for (const session of halls) {
      const hall = grouped.get(session.hallId);
      if (hall) {
        hall.sessions.push(session);
      } else {
        grouped.set(session.hallId, {
    ...
    return Array.from(grouped.values()).map((hall) => ({
      ...hall,
      totalSessions: hall.sessions.length,
    }));
  }, [halls]);
  ...
  const handleSendGeneralNotify = async (item: ActiveHallSummary) => {
    try {
      const all = await getBachelorsByHallSession(
        item.hallId,
        item.sessionId
      );
