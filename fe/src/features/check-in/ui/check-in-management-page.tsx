...
import { checkInKeys } from '@/features/check-in/queries/check-in-query-options';
import { useCheckInListQuery } from '@/features/check-in/queries/use-check-in-list-query';
...
export default function CheckInManagementPage() {
  ...
  const createNotificationMutation = useMutation({
    ...
    onError: (error: unknown) => {
      toast.error(getHttpErrorMessage(error, 'Không thể gửi thông báo. Vui lòng thử lại.'));
    },
  });
  ...
  const toggleMutation = useMutation({
    ...
    onMutate: async (vars) => {
      ...
      const prev = queryClient.getQueryData<CheckinRow[]>(checkInKeys.list());
    ...
    onError: (_err, _vars, ctx) => {
      // Rollback
      if (ctx?.prev) {
        queryClient.setQueryData(checkInKeys.list(), ctx.prev);
      }
      toast.error('Cập nhật trạng thái thất bại.');
    },
    onSuccess: (_data, vars) => {
      // Refresh to be exact with server
      queryClient.invalidateQueries({ queryKey: checkInKeys.list() });
