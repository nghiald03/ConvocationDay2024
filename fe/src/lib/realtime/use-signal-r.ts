...
interface UseSignalROptions<TBroadcast> {
  hubUrl: string;
  autoConnect?: boolean;
...
export const useSignalR = <TBroadcast = unknown>(
  options: UseSignalROptions<TBroadcast>
) => {
  const {
    hubUrl,
    autoConnect = true,
  ...
  const setStateSafe = useCallback((s: string) => {
  ...
  const joinNoticerGroupInternal = useCallback(
    async (entry: SRRegistryEntry) => {
      ...
      const connected = await waitForConnected(entry);
      if (!connected) {
      ...
      try {
        entry.joining = true;
        await entry.conn.send('JoinNoticerGroup');
        entry.joined = true;
        console.log('[SignalR] Joined Noticer group (send)');
      } catch (error) {
        entry.joined = false;
        console.error('[SignalR] JoinNoticerGroup failed:', error);
      } finally {
        entry.joining = false;
      }
    },
  ...
  const ensureEntry = useCallback(() => {
    ...
    if (!entry) {
      ...
      const urlOpts: IHttpConnectionOptions = { withCredentials: true };
      if (forceWebsockets) {
    ...
    return entry;
  }, [registry, hubUrl, forceWebsockets, onTTSBroadcast, joinNoticerGroupInternal, setStateSafe]);
  ...
  const startConnection = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      setStateSafe('Disconnected');
      setIsConnected(false);
      console.error('[SignalR] start failed:', err instanceof Error ? err.message : err);
      return false;
    } finally {
      entry.starting = false;
    }
  }, [ensureEntry, hubUrl, setStateSafe, waitForConnected]);
  ...
  }, [hubUrl, autoConnect, forceWebsockets, stopDelayMs]);
